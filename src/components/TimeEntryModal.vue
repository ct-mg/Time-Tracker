...
function save() {
    ...
    if (endTime.value && new Date(startTime.value) > new Date(endTime.value)) {
        toastStore.error(t('ct.extension.timetracker.common.endAfterStart'));
        return;
    }
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg w-full">
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                        {{ entry ? t('ct.extension.timetracker.manualEntry.updateEntry') : t('ct.extension.timetracker.timeEntries.addManualEntryTitle') }}
                    </h3>
                    <div class="mt-4 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('ct.extension.timetracker.timeEntries.startTime') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('ct.extension.timetracker.timeEntries.endTime') }}</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-1">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('ct.extension.timetracker.dashboard.category') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('ct.extension.timetracker.dashboard.description') }}</label>
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="is-break" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">
                                {{ t('ct.extension.timetracker.manualEntry.isBreakLabel') }}
                            </label>
                        </div>
                    </div>
                </div>
